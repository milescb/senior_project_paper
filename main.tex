...
Now that we have defined our problem, we can use standard minimization techniques to find a solution. For a complete example see Appendix \ref{ODE_example} and for additional information and further examples see \cite{rackauckasSciMLSciMLBookParallel}. 
...
For a simple implementation using \texttt{NeuralPDE.jl} see Appendix \ref{neural_PDE_eg} and for a more in-depth example see Appendix \ref{neural_PDE_eg2}. 
...
One of the greatest challenges in solving this problem is managing physical memory of the system. When adding an additional loss term to the solving scheme, each operation added by the user takes certain amount of memory. In particular, when performing back-propagation over a differential equation solver in addition to attempting to solve a large system of ODEs results in huge memory allocation. If too much memory is required in order to solve, the system crashes and a segmentation fault is reported. In order to try to solve this problem, we do all calculations with 32-bit floats rather than the standard 64-bit. Additionally, we cannot ask the ODE solver in the additional loss function to solve for many points: around 15 points seems to be the limit. In this solving, all attempts to automatically interpolate must be turned off so as not to waste memory. 
